    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover">
            ...
            <tr>
                ...
                <td class="text-center">
                    <a href="javascript:sort('goods_state');">状态</a>
                </td>                
            ...
            <?php if(is_array($goodsList)): $i = 0; $__LIST__ = $goodsList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td class="text-left">
                    <input type="text" onkeyup="this.value=this.value.replace(/[^\d.]/g,'')" onpaste="this.value=this.value.replace(/[^\d.]/g,'')"  onchange="ajaxUpdateField2(this);" name="store_count" size="4" data-table="goods" data-id="<?php echo ($list["goods_id"]); ?>" value="<?php echo ($list["store_count"]); ?>"/>   
                    </td>                    
                    <td class="text-center">                         
                    ...
                    <?php if($list[goods_state] == 1): ?>出售中<?php endif; ?>
                    <?php if($list[goods_state] == 2): ?>审核失败<?php endif; ?>
                    <?php if($list[goods_state] == 3): ?>违规下架<?php endif; ?>                    
                    </td>                                                 
                    <td class="text-right">
                        <a href="<?php echo U('Goods/addEditGoods',array('goods_id'=>$list['goods_id']));?>" data-toggle="tooltip" title="" class="btn btn-primary" data-original-title="编辑"><i class="fa fa-pencil"></i></a>
                        <a href="javascript:void(0);" onclick="del('<?php echo ($list[goods_id]); ?>')" id="button-delete6" data-toggle="tooltip" title="" class="btn btn-danger" data-original-title="删除"><i class="fa fa-trash-o"></i></a>
