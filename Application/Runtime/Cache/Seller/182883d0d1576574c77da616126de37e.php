                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover">
                                ...
                                <?php if(is_array($orderList)): $i = 0; $__LIST__ = $orderList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
										...
										<td class="text-center"><?php echo (date('Y-m-d H:i',$list["pay_time"])); ?></td>
                                        <td class="text-center"><?php echo ($list["total_amount"]); ?></td>
                                         <td class="text-center">
                                        	<?php if($list['shipping_status'] != 1): ?><a href="<?php echo U('Seller/order/delivery_info',array('order_id'=>$list['order_id']));?>" data-toggle="tooltip" title="" class="btn btn-info" data-original-title="查看详情">去发货</a>
                                        	<?php else: ?>
                                        		<a href="<?php echo U('Seller/order/delivery_info',array('order_id'=>$list['order_id']));?>" data-toggle="tooltip" title="" class="btn btn-info" data-original-title="查看详情"><i class="fa fa-eye"></i></a>
                                        		<a href="<?php echo U('Seller/Order/shipping_print',array('order_id'=>$list['order_id']));?>" target="_blank" data-toggle="tooltip" class="btn btn-default" title="打印快递单">
