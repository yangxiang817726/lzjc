...
<!--                        <li><a href="#tab_goods_desc" data-toggle="tab">描述信息</a></li>-->
                        ...
                        <li><a href="#tab_goods_attr" data-toggle="tab">属性</a></li>
                        ...
                        <div class="tab-pane active" id="tab_tongyong">
                            ...
                            <table class="table table-bordered">
                                ...
                                <tr>
                                    <td>佣金:</td>
                                    <td>
                                        <input type="text" value="<?php echo ($goodsInfo["distribut"]); ?>" name="distribut" class="form-control" style="width:150px;" onkeyup="this.value=this.value.replace(/[^\d.]/g,'')" onpaste="this.value=this.value.replace(/[^\d.]/g,'')" />
                                        	用于分销的分成金额
                                    </td>
                                </tr>                                  
                                <tr>
                                ...
                                <tr>
                                    <td>商品详情描述（逗号隔开 举例  长:3米，宽5米）:</td>
                                    <td width="85%">
										<textarea class="span12 ckeditor" style="width:550px;" id="goods_content" name="goods_content" title="" ><?php echo ($goodsInfo["goods_content"]); ?></textarea>
                    ...
                    <div class="pull-right">
                        ...
                        <button class="btn btn-primary" onclick="ajax_submit_form('addEditGoodsForm','<?php echo U('Goods/pubptGoods?is_ajax=1');?>');" title="" data-toggle="tooltip" type="button" data-original-title="确认发布">确认发布</button>
                    </div>
			    </form><!--表单数据-->
                </div>
            </div>
...
* ajax 加载规格 和属性
*/
function ajaxGetSpecAttr(goods_id,cat_id2)
{
	// ajax调用 返回规格
	$.ajax({
			type:'GET',
//			data:{goods_id:goods_id,cat_id2:cat_id2},
			url:"/index.php?m=Seller&c=Goods&a=ajaxGetSpecSelectpt&goods_id="+goods_id+"&cat_id2="+cat_id2,
			success:function(data){                            
				   $("#ajax_spec_data").html('');
				   $("#ajax_spec_data").append(data);
				if($.trim(data) != '')	
				   ajaxGetSpecInputpt();	// 触发完  马上处罚 规格输入框
			}
	});	
	...
	$.ajax({
			type:'GET',
//			data:{goods_id:goods_id,cat_id2:cat_id2},
			url:"/index.php?m=Seller&c=Goods&a=ajaxGetAttrInput&goods_id="+goods_id+"&cat_id2="+cat_id2,
			success:function(data){
...
$(document).ready(function(){
	...
	// 商品分类第三个下拉菜单	
	// <?php if($goodsInfo['cat_id3'] > 0): ?>// 	 get_category2("<?php echo ($goodsInfo['cat_id2']); ?>",'cat_id3',"<?php echo ($goodsInfo['cat_id3']); ?>");
	//<?php endif; ?>
	...
	// 如果是编辑的时候 加载 规格 和属性
	<?php if($goodsInfo['cat_id2'] > 0): ?>ajaxGetSpecAttr(<?php echo ($goodsInfo['goods_id']); ?>,<?php echo ($goodsInfo['cat_id2']); ?>);<?php endif; ?>
...
$("#cat_id2").change(function(){
		...
		if(cat_id2 == 0)
		 return false;
		 ajaxGetSpecAttr(goods_id,cat_id2);
});
